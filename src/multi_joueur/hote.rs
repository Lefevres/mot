...
#[tokio::main]
pub async fn hote(mode: Mode, affichage : &Box<dyn Affichage>){

    let nb_client:usize= demander_nb_joueur(&affichage);

    let (joueur, liste, mon_nom, nb_manche) = se_préparer("hote", affichage);

    let clients = connextion_au_client(nb_client, &affichage).await.unwrap();
    ...
    match mode {
        ...
        Mode::Classique => {
            option = true;
            info = demander_nb_manche(liste.len(), affichage);
            envoi_message_tous(&mut sockets,&info.to_string()).await;
        }
        Mode::Chronomètre => {
            option = true;
            info = demander_temp(affichage);
            envoi_message_tous(&mut sockets,&info.to_string()).await;
        }
        _ => ()
    }
   // message_initialisation(&mut sockets, nb_manche, &liste[0..nb_manche].to_vec(),mode.clone()).await;  //fois deux pour question réponse; faire très attention si jouer, tester le multi
    ...
    jeux.jouer(if option { Some(info) } else { None }, &affichage);

    résultats = met_a_jour_les_résultats(&mut sockets,&joueur).await;

    afficher_résultat(nb_client, &noms, mon_nom, &résultats, &affichage);
...
fn afficher_résultat(nb_client:usize, noms :&Vec<String>, mon_nom :String, résultats :&Vec<(String,String)>, affichage : &Box<dyn Affichage>) {
    ...
    for i in 0..nb_client+1 { //pour l'hote
        ...
        affichage.afficher(format!("{} a eu {} bonne réponse(s) pour {} mauvaise(s) pour un ration de {:.1}% \n",nom, résultats[i].0, résultats[i].1,ratio));
    }
}
...
fn demander_nb_joueur(affichage : &Box<dyn Affichage>) -> usize {
    affichage.afficher_str("Pour combien de joueur ? (hormis toi)");
    loop {
...
async fn connextion_au_client(nb_client: usize, affichage : &Box<dyn Affichage>) -> Result<(Vec<String>,Vec<TcpStream>), Box<dyn std::error::Error>> {
    let listener = TcpListener::bind("0.0.0.0:9000").await?;
    let mut noms_joueurs = Vec::new();
    let mut sockets = Vec::new();
    for _ in 0..nb_client {
        if nb_client > 1 {
            affichage.afficher_str("En attente des joueurs...");
        }else{
            affichage.afficher_str("En attente du joueur...");
        }

        let (mut socket, adresse) = listener.accept().await?;
        affichage.afficher(format!("Client connecté : {}", adresse));
